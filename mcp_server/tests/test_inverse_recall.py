...
The inverse problem: start from KNOWN-GOOD places, each paired with the
query an agent *should* plausibly issue to find it and the anchor it would
naturally use. Recall = the fraction of seeds actually returned by their
own inverse query. Where `test_live_smoke.py` checks shape ("at least one
result"), this measures retrieval QUALITY — a schema or query-construction
regression shows up here as a recall drop long before any shape test fails.

INDEPENDENCE IS THE POINT of the seed set. Seeds must NOT be generated by
running category sweeps through `search_nearby_places` and keeping what
comes back — that collapses the test into "the query returns what the same
query returned", which stays green no matter how bad retrieval is. The
current seeds were sourced from the assisting model's training knowledge
of well-regarded NYC venues (a compressed aggregate of editorial and
cultural consensus, independent of this tool's ranking), with each inverse
query written BEFORE any lookup. Candidates were then verified alive via a
NAME lookup — a different retrieval path than the category queries under
test — and kept only if rating > 4.5 with >= 200 ratings (which drops even
famous places: Katz's and Death & Co sit at exactly 4.5). Editorial lists
(Michelin, Eater, Infatuation) are an equally valid independent source for
future reseeding; the tool's own category output is not.
...
Event seeds come from one `get_events` batch over venues that post
schedules; they carry start_date and are skipped once past. The events leg
measures extraction drift (site redesigns, scraper breakage) rather than
search ranking, so tool-discovered venues are acceptable there.
"""
